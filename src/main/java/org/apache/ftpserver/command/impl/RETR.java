...
import com.exc_led.ftpserver.biz.NodeBiz;
...
public class RETR extends AbstractCommand {
	...
	private static NodeBiz nodeBiz;

	@Autowired
	public void setJedisPool(JedisPool jedisPool) {
		RETR.jedisPool = jedisPool;
	}

	@Autowired
	public void setNodeBiz(NodeBiz nodeBiz) {
		RETR.nodeBiz = nodeBiz;
	}
		...
		try {
			...
			if (fileName == null) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_501_SYNTAX_ERROR_IN_PARAMETERS_OR_ARGUMENTS, "RETR", null, null));
				return;
			}
			...
			if (file == null) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "RETR.missing", fileName, file));
				return;
			}
			...
			if (!file.isFile()) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "RETR.invalid", fileName, file));
				return;
			}
			...
			if (!file.isReadable()) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "RETR.permission", fileName, file));
				return;
			}
			...
			// get data connection
			session.write(LocalizedFtpReply.translate(session, request, context, FtpReply.REPLY_150_FILE_STATUS_OKAY,
					"RETR", null));
			...
			try {
				...
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_425_CANT_OPEN_DATA_CONNECTION, "RETR", null, file));
				return;
			}
			...
			try {
				...
				failure = true;
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_426_CONNECTION_CLOSED_TRANSFER_ABORTED, "RETR", fileName, file, transSz));
				// 当客户端下载过程中主动断开连接时,获取信息并且通知物联网
				String pureFileName = fileName.substring(fileName.lastIndexOf("/") + 1, fileName.length());
				// 取得ip地址
				String hostName = session.getFtpletSession().getClientAddress().getHostName();
				System.out.println("hostName:"+hostName);
				String num = nodeBiz.findNumByIp(hostName);
				String setName = pureFileName + "-" + num;
				// redis发布
				Jedis jedis = jedisPool.getResource();
				jedis.set(setName, Integer.toString(2));
				jedis.publish(setName, setName);
				jedis.close();
				System.out.println("jedis发布的KEY:"+setName);
				System.out.println("jedis发布的VALUE:"+setName);
			} catch (IOException ex) {
				LOG.debug("IOException during data transfer", ex);
				failure = true;
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_551_REQUESTED_ACTION_ABORTED_PAGE_TYPE_UNKNOWN, "RETR", fileName, file,
						transSz));
			} finally {
